#!/usr/bin/env bash
# Create and push an annotated semver tag. Mirrors translaas-sdk-js/scripts/create-coordinated-release-tag.mjs.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h | --help)
      ...
      echo "  --dry-run Validate CHANGELOG and tag name without creating or pushing"
      exit 0
      ;;
    -*)
...
if echo "$remote_tags" | grep -q "refs/tags/${tag}"; then
  echo "Tag $tag already exists on origin; skipping coordinated release tag."
  exit 0
fi
